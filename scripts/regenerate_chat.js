...
function question() {
  const pools = {
    sign: core.sign,
    move: ['천천히 이동', '잠깐 정지', '재확인', '우회', '진입 보류', '속도 낮추기', '동선 변경', '뒤로 빠지기', '관찰 유지', '호흡 정리'],
    phase: ['탐색 중', '확인 중', '정리 중', '판단 중', '대기 중', '이동 중', '체크 중', '관찰 중', '신중 모드', '위기 대응 중'],
    __addon1: ['지금은 화면보다 판단 흐름이 더 중요해', '채팅도 같은 타이밍에서 고민 중이야', '이 구간은 한 번 더 확인하는 게 맞아 보여', '정보가 아직 덜 모인 상태 같아', '분위기상 성급하게 가면 손해 볼 듯', '지금 선택 하나가 흐름 바꿀 수 있어', '이 장면은 판단 미스가 자주 나와', '속도보다 정확도가 중요해 보임'],
    __addon2: ['한 템포 늦춰도 괜찮아', '지금은 안정적으로 가는 게 맞아', '먼저 뭐 하는 중인지부터 정리하자', '다음 행동 합의하고 가자', '잠깐 멈추고 다시 보자', '체크리스트 느낌으로 가면 좋아', '지금은 과감함보다 안전이 우선이야', '리스크 낮추는 선택이 나아 보인다'],
  };
  ...
  const templates = [
    ...
    (v) => `지금 나 어디 가는 중이지?`,
    (v) => `지금 뭐 하는 중으로 보면 돼?`,
    (v) => `다음 행동은 ${v.move} 쪽이 맞아 보이는데 동의해?`,
    (v) => `지금 상태가 ${v.phase} 맞지?`,
    (v) => `이 타이밍에 밀어도 돼, 아니면 잠깐 멈출까?`,
    (v) => `지금 템포 유지하는 게 맞아?`,
    (v) => `흐름상 지금은 공격적으로 가도 되는 구간이야?`,
    (v) => `방금 ${v.sign} 들린 거 나만 그런 거 아니지?`,
    (v) => `${v.sign} 거리감 갑자기 달라진 느낌인데 맞아?`,
    (v) => `지금은 확인 먼저 하고 가는 게 낫겠지?`,
    (v) => `이 장면은 후퇴 선택도 열어두는 게 맞아 보여?`,
    (v) => `지금 바로 진행하면 리스크 큰 편이야?`,
    (v) => `이 패턴 트리거 같아, 아니면 페이크 같아?`,
    (v) => `채팅 기준으로 지금 위험도 몇 점 정도야?`,
    (v) => `이 구간은 속도보다 정확도 우선 맞지?`,
    (v) => `지금 판단 한 번만 더 점검해도 될까?`,
    (v) => `지금 페이스로 가면 대응 가능해 보여?`,
    (v) => `현재는 멈춤이 맞나요, 진행이 맞나요?`,
    (v) => `다음 행동은 신중하게 가는 게 적절할까요?`,
  ];
